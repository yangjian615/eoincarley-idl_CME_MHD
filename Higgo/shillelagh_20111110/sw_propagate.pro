...
pro sw_propagate, intime, trange=intrange, tbin=intbin, savpath=insavpath, no_alpha=no_alpha, full360=full360, $
	...
	custom=fcustom, nostereo=nostab
...
;Interpolate points to create solution images
	...
	if not keyword_set(test) then save,sw_arrays,file=savpath+'sw_properties_points_'+time2file(anytim(thistime,/vms))+'_'+filetag+'.sav',/compress
