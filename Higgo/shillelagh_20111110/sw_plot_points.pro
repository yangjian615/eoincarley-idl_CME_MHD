...
pro sw_plot_points, time, array, file=infile, velocityplot=velocityplot, densityplot=densityplot, $
	...
	savpath=insavpath, _extra=_extra, outearthpos=outearthpos, nodata=nodata, label_planet=label_planet, $
	fontcode=fontcode, no_plot_sc=no_plot_sc, outarr=spirals, notitle=notitle
...
case propnum of 
	...
	3: color_table, densrange,cbarpos[[0,2]],cbarpos[[1,3]],title='LOG n [cm'+textoidl('^{-3}')+']';,/shadow
...
if n_elements(footpoints) gt 0 then begin
	...
	for i=0,n_elements(footpoints)-1 do begin
		...
		oplot,footpos_x,footpos_y,color=!gray
