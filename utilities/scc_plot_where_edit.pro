...
;-
;
pro scc_plot_where_edit, date, inner=inner, outer=outer, ulysses=kulysses, $
                    messenger=kmessenger, vesta=kvesta, ceres=kceres, $
                    parker=parker, pos=pos, white_bg=white_bg, _extra=_extra
on_error, 2
;
...
if twin_plots then begin
    ...
    setscale, xmin*1.1, xmax*1.1, ymax, ymin, /noadjust
    plot, x, y, /nodata, xtitle='X', ytitle=' Y', charsize=1, $
    position=pos, /normal, /noerase, xstyle=4, ystyle=4
endelse
;
...
if (xx ge !x.crange[0]) and (xx le !x.crange[1]) and (yy le !y.crange[0]) and $
  (yy ge !y.crange[1]) then begin
    plots, xx, yy, psym=8, color=grey, symsize=0.5
    if not keyword_set(outer) then xyouts, xx, yy, ' Mercury', color=grey, charsize=0.5
endif
;
...
if (xx ge !x.crange[0]) and (xx le !x.crange[1]) and (yy le !y.crange[0]) and $
  (yy ge !y.crange[1]) then begin
    plots, xx, yy, psym=8, color=grey, symsize=0.5
    if not keyword_set(outer) then xyouts, xx, yy, ' Venus', color=grey, charsize=0.5
endif
;
...
plots, symsize=1, 0, earth[0], psym=8, color=green
if not keyword_set(outer) then xyouts, 0, earth[0], '  Earth', color=green, charsize=0.7, charthick=1
;
;  Overplot the Sun.
;
plots, symsize=2, 0, 0, psym=8, color=yellow
if not keyword_set(outer) then xyouts, 0, 0, '  Sun', color=yellow, charsize=0.7, charthick=1
;
;  Overplot the positions of the two STEREO spacecraft.  Draw lines from the
;  Sun through each spacecraft.
;
oplot, [0,1.2*sta[1]], [0,1.2*sta[0]], line=1
oplot, [0,1.2*stb[1]], [0,1.2*stb[0]], line=1
plots, symsize=0.5, sta[1], sta[0], psym=8, color=red
if not keyword_set(outer) then xyouts, sta[1]+0.05, sta[0], 'A', charsize=1, charthick=1, $
  color=red
plots, symsize=0.5, stb[1], stb[0], psym=8, color=blue
if not keyword_set(outer) then xyouts, stb[1]+0.05, stb[0], 'B', charsize=1, charthick=1,$
  color=blue
;
;  Reset the scale.
;
