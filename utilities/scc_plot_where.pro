...
;-
;
pro scc_plot_where, date, inner=inner, outer=outer, ulysses=kulysses, $
                    messenger=kmessenger, vesta=kvesta, ceres=kceres, $
                    parker=parker, white_bg=white_bg, _extra=_extra
on_error, 2
;
...
plots, symsize=3, 0, earth[0], psym=8, color=green
if not keyword_set(outer) then xyouts, 0, earth[0], '  Earth', color=green, charsize=3
;
;  Overplot the Sun.
;
plots, symsize=5, 0, 0, psym=8, color=yellow
if not keyword_set(outer) then xyouts, 0, 0, '  Sun', color=yellow, charsize=3
;
...
if not keyword_set(outer) then xyouts, sta[1], sta[0], ' A', charsize=3, $
  color=red
plots, symsize=2, stb[1], stb[0], psym=8, color=blue
if not keyword_set(outer) then xyouts, stb[1], stb[0], ' B', charsize=3, $
  color=blue
;
;  Reset the scale.
;
