...
FOR i = 0, n_elements(wave[*,0])-1 DO BEGIN
	index = where(period gt coi[i]) 
	wave[i, index] = !values.f_nan
	wave_z[i, index] = 0.0
ENDFOR	

power = average(abs(wave_z),1)
freq = 1/period

PLOTSYM, 0               ;Plotting symbol is a circle

plot, power, period, thick=5, position=[0.71, 0.17, 0.84, 0.38], /normal, /noerase, /xlog, /ylog, xr=[0.1,50], $
yr=[100, 0.1], /xs, /ys, xtitle=' ',$
ytitle=' ', xticklen=0.001, ytickname=[' ',' ',' ',' ',' '],$
xtickname=[' ',' ', ' '], yticklen=0.1

axis, xaxis=1, /xlog, xr=[0.1,50], /xs, xtitle='Power', xticklen=0.05
axis, yaxis=1, /ylog, yr=[100, 0.1], ytitle='Period (s)', yticklen=0.05

period = (dindgen(101)*(100.0 - 0.1)/100.0 ) +0.1
oplot, 2.7*period^0.66, period,  linestyle=0, color=5, thick=3
;xyouts, 0.76, 0.35, '2.7 x T!U0.66!N', /normal
