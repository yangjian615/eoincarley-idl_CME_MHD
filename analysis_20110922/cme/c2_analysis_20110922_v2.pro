pro c2_analysis_20110922_v2,create_rtheta_space = create_rtheta_space
...
IF keyword_set(create_rtheta_space) THEN BEGIN
  ...
  r_theta_t = dblarr(3,17,3)
	FOR j=4,6 DO BEGIN
	  ...
	  data_bs = data_bs/stdev(data_bs)
	  data_bs_scl = data_bs >(-4.0)<4.0
	  ...
	  plot_image,data_bs_scl
		...
		FOR i = angle1, angle2, nsteps DO BEGIN
			...
			sun = scc_SUN_CENTER(he)
   			...
   			print,'Angle: '+string(i)+', radius: '+string(r1)
			r_theta_t[0, *, j-4] = anytim(he.date_obs,/utim)
			r_theta_t[1, abs(i-angle1)/nsteps, j-4] = r1
			r_theta_t[2, abs(i-angle1)/nsteps, j-4] = i
	...
	ENDFOR
save,r_theta_t,filename='c2_20110922_r_theta_t_test.sav'
