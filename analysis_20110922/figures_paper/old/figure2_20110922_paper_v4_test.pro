pro figure2_20110922_paper_v4_test
...
pro overplot_nrh, nrh_times, nrh_data, nrh_hdr, he_aia, c_color, pos, tag, c2_hdr_struc, lasco_label, lin
	...
	levels = (dindgen(10.0)*(max_val - max_val*0.9)/9.0) + max_val*0.9
	set_line_color 
	plot_map, map_nrh, /overlay, /cont, levels=levels, c_color=c_color, $
		/noxticks, /noyticks, xticklen=0.0, /noaxes, thick=5
	;plot_map, map_nrh, /overlay, /cont, levels=levels, c_color=0, $
	;	/noxticks, /noyticks, xticklen=0.0, /noaxes, linestyle=lin, thick=0.5
	angstrom='A'
	text_size=1.0
IF tag eq 'a' THEN BEGIN
