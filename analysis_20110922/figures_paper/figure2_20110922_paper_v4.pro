...
cd,'~/Data/22sep2011_event/NRH/NRH_AIA_pdf'

set_plot,'ps'
!p.font=0
device,filename = 'AIA_NRH_figure_v2_test.ps', /color, /inches, /landscape, /encapsulate,$
yoffset=13,ysize=8,xsize=13, bits_per_pixel=8, /helvetica
;window,1,xs=1300,ys=1000
;------------------------------------------------------------;
;[x1,y1,x2,y2]
FOV = [40.0,40.0]
CENTER = [-1300.0, 50.0]
...
;Use these image times if max is scaled to each image.
...
pro create_composite, image_num, pos, FOV, CENTER, tag, occulter, lasco_label, yaxis=yaxis, xaxis=xaxis
	...
	;map_aia_pre.data = smooth(map_aia_pre.data, 3)
	;map_aia.data = smooth(map_aia.data, 3)
	...
	dmin = -7.0, dmax = 5.0, /noaxes  ;usually dmin=-6, dmax=3
	set_line_color
	...
	IF tag eq 'c' THEN BEGIN
		plot_circles, 1.47
		xyouts, -1600, 800, '1.47 R!Lsun!N', color = 7, /data, charsize=1.0, charthick=3
		plot_circles, 2.1
		xyouts, -1650, 1100, '2.1 R!Lsun!N', color = 7, /data, charsize=1.0, charthick=4
	ENDIF
...
pro overplot_nrh, nrh_times, nrh_data, nrh_hdr, he_aia, c_color, pos, tag, c2_hdr_struc, lasco_label, lin
	...
	;cgCOLORBAR, bottom=70, ncolors=150, position=[0.94, 0.01, 0.945, 0.99], charsize=1.0, /vertical,$
	;yticks=3, ytickname=['7.5', '8.0', '8.5', '9.0'], /right, yticklayout=1
