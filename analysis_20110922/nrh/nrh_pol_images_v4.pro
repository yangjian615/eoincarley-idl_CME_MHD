...
; v4 plot movie but percent polarized

;------------- Define times ---------------;

t1 = anytim(file2time('20110922_103500'),/utim)
t2 = anytim(file2time('20110922_103650'),/utim)
...
max_I_loc_pos = dblarr(n_elements(nrh_data_I[0,0,*]))
max_I_loc_neg = dblarr(n_elements(nrh_data_I[0,0,*]))
...
FOR i=0, n_elements(nrh_data_I[0,0,*])-1 DO BEGIN
	...
	;--------------------------------------------------------;
	; 			   Get percent polarization
	
	;Get the value of total I at the location of max positive V
	index_pos = where(map_nrh_V.data eq max(map_nrh_V.data))
	nrh_I = map_nrh_I.data
	xy = array_indices(nrh_I, index_pos)
	max_I_loc_pos[i] = nrh_I[xy[0], xy[1]]
	max_V_pos[i] = max(map_nrh_V.data)
	percent_pos_pol = (max_V_pos/max_I_loc_pos)*100.0
	
	
	;Get the value of total I at the location of max negative V
	index_neg = where(map_nrh_V_neg.data eq max(map_nrh_V_neg.data))
	nrh_I = map_nrh_I.data
	xy = array_indices(nrh_I, index_neg)
	max_I_loc_neg[i] = nrh_I[xy[0], xy[1]]
	max_V_neg[i] = max(map_nrh_V_neg.data)
	percent_neg_pol = (max_V_neg/max_I_loc_neg)*100.0
	...
	;
	!p.multi=[0,1,1]
	utplot, times, percent_pos_pol, xr = [t1, t2], /xs, charsize=1.5, yr=[0, 100], $
	position=[0.1, 0.1, 0.98, 0.45], /normal, /noerase,  ytitle='Percent Circularly Polarized',$
	color=5
	...
	oplot, times, percent_neg_pol, color=3, psym=3
	
	legend, ['Right Circular','Left Circular'], linestyle=[0,0], color=[5,3], box=0,$
	charsize=1.5
;x2png,nrh_times_I[i]+'_percent.png'	
;stop
	
ENDFOR	

;save, times, max_I_loc_pos, max_I_loc_neg, max_V_pos, max_V_neg, filename='percent_pol.sav'
