...
t2 = anytim(file2time('20110922_103510'),/utim)
...
FOR i=0, n_elements(nrh_data_I[0,0,*])-1 DO BEGIN
	...
	position=[0.1, 0.55, 0.5, 0.95], /normal, /noerase
	
	xyouts, 0.11, 0.92, 'Stokes I (Total Intensity)', /normal, charsize=2
	...
	position=[0.58, 0.55, 0.98, 0.95], /normal, /noerase
	...
	;
	loadct,3
	tv, bytscl( congrid(map_nrh_V_neg.data, 400, 400), -1e7, 5e7), channel=1, 0.58, 0.55, /normal
	plot_helio, nrh_times_I[i], /over, gstyle=0, gthick=1.0, gcolor=254, grid_spacing=15.0
	
	set_line_color
	xyouts, 0.6, 0.92, 'Right Circular', /normal, charsize=2, color=5
	xyouts, 0.6, 0.9, 'Left Circular', /normal, charsize=2, color=3
	
	;xyouts, 0.6, 0.80, 'Stokes V<0 (Left)', /normal, charsize=2, color=1
	;x2png,nrh_times_I[i]+'L.Rpng'
	...
	;
	!p.multi=[0,1,1]
	utplot, times, max_I, xr = [t1, t2], charsize=2, /ylog, yr=[1e5, 1e9], $
	...
	oplot, times, max_V_pos, color=5s
