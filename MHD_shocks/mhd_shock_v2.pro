...
IF type eq 'oblique' THEN BEGIN
    ...
    ;
    ;( (gam+chi[*]*(2.0-gam))*vu^2.0 - ((gam+1.0)-chi[*]*(gam-1.0))*chi[*]*va^2.0 )

	L = (Ma^2.0 -chi[*])^2.0
	M = ( gam*p_beta*chi[*]  + Ma^2.0 * cos(theta*!DTOR)^2.0 *( (gam-1.0)*chi[*] - (gam+1.0)  ))
	N = Ma^2.0 *sin(theta*!DTOR)*chi[*]
	O = ( (gam +  (2.0-gam)*chi[*] )*Ma^2.0   - chi[*]*(  (gam+1.0)   -chi[*]*(gam-1.0)   )  )    
...
IF type eq 'perp' THEN BEGIN
	...
	;
	Ms = Ma*sqrt(2.0/(gam*p_beta)) ;to convert from Alfven to Sonic Mach
	polyn = fltarr(n_elements(chi))
	polyn[*] = 2.0*(2.0-gam)*chi[*]^2.0 + $
	(2*p_beta + (gam-1.0)*p_beta*Ms^2.0 + 2.0)*gam*chi[*] - $
	gam*(gam+1.0)*p_beta*Ms^2.0
ENDIF
...
pro mhd_shock_v2,type,VARY_THETA=vary_theta,VARY_MACH=vary_mach
...
;********Define constants for 1.55 Rsun***********
;This is to be used in determination of Mach number of split band type II from 22-Sep-2011
;It starts at ~45 MHz ->1.55 Rsun
proton_mass = 1.67e-27
kb = 1.38e-23		 	   	;Boltzman constant (SI)
mu = 1.26e-6			   	;magnetic permeability (SI)
gam = 5.0/3.0				;adiabatic index (ideal gas)

n_cm = 7.0e7           		;number density (cm^-3) (75 MHz)
n_m = 1.0e6*n_cm 	   		;number density (m^-3)
Temp = 1.4e6           		;Temperature (K)
therm_p = 2.0*n_m*kb*Temp	;thermal pressure (ideal gas law)

B_gauss = 0.9	   			;magnetic field stregth (c.g.s) ;2.2/(1.34^2.0) 
							;2.2G at surface and 1/r^2.0 drop-off, r=1.55 (height of 45 MHz from Newkirk)
...
print,'Alfven speed :'+string(va/1.0e3)+' km/s'
cs=sqrt((gam*therm_p)/(n_m*proton_mass))  ;Sound speed
stop

;************************************

theta = 0.0					;Angle between flow and shock normal. Kept constant if /vary_mach
...
result=chi_polynomial(Ma, theta, p_beta, va, cs, gam, type = type) ;see function above
...
;*************Plot********************
plot,result[0,*],result[1,*],color=0, yr=[-10,10],xr=[0,10],$
...
window,1
plot, sqrt(mach_v_comp[1,*])-1.0, mach_v_comp[0,*], psym=1, yr=[1,5], xr=[0.0,1.0];, xtitle='Mach Number', ytitle='Shock Compression',$
charsize=1.5
;chi=mach_v_comp[1,*]
;Ma=mach_v_comp[0,*]
;save,chi,Ma, filename='ma_v_chi_beta1.sav'

comp=1.69
mach_est = interpol(mach_v_comp[0,*], mach_v_comp[1,*], comp)
print,'For compression of '+string(comp, format='(f5.2)')+', Mach = '+string(mach_est)
print,'Plasma beta parameter: '+string(p_beta)
