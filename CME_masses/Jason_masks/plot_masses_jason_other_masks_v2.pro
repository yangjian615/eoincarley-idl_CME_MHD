pro plot_masses_jason_other_masks_v2, toggle = toggle
...
IF keyword_set(toggle) THEN BEGIN
	set_plot,'ps'
	device,filename='mask_nomask_invmask_c2c3.ps', /color, /inches, /encapsulate, $
	ysize=20, xsize=20
ENDIF
...
set_line_color	   		  	   		  
!p.multi=[0,1,1]
utplot, t_c2, mask_mass2/C, xr=[tstart, tend], yr=[-2, 6], $;(max_mass + 0.5*max_mass)/C+0.5], $
...
oplot, t_c2, mass_c2/C, color=3, thick=6
;oplot, t_c3, mask_mass3/C, color=6, thick=6

legend,['Mask','No Mask - Inverse Mask'], linestyle=[0,0], charsize=2.0, box=0, color=[5,3]
...
utplot, t_c3, mask_mass3/C, xr=[tstart, tend], yr=[-2, 6], $;(max_mass + 0.5*max_mass)/C+0.5], $
/xs, /ys, ytitle='CME Mass x1e15 (g)' ,color=5, charsize=2.0, thick=6,$
	title='CME Mass v Time: Mask, No Mask - Inverse Mask (C3)', position=[0.07, 0.07, 0.95, 0.45],$
	/noerase, /normal
	;
	;position=[0.12, 0.68, 0.99, 0.99], xtickname=[' ',' ',' ',' ',' ', ' '],xtit=' '
...
oplot, t_c3, mass_c3/C, color=3, thick=6
;oplot, t_c3, mask_mass3/C, color=6, thick=6
legend,['Mask','No Mask - Inverse Mask'], linestyle=[0,0], charsize=2.0, box=0, color=[5,3]
;legend,['Mask', 'No Mask', 'Invert Mask'], linestyle=[0,0,0], color=[5,6,4], /right, box=0
