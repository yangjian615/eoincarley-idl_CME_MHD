function massCalculator,img,ha,MAXSCL=maxscl,MINSCL=minscl,SAVE=save, $
         SECTOR=SECTOR,RADII=RADII,ANGLES=ANGLES
;+
; 
;  N.B This is a modified version of scc_cme_massimg2total so that the intereactive display window is not
;  shown and the only output is the mass
   ;NAME:
...
;IF (nx gt 1024) or (ny gt 1024) THEN BEGIN
 ;xloadct
  ;SLIDE_IMAGE,bytscl(a,min=mn,max=mx),xvisible=1000,yvisible=700,show_full=0,slide_window=sw_a
  ;wset,sw_a
  ;junk = ''
...
ii = 0.0
maxPIX = max(sigrange(mass))


; This loop is not in scc_cme_massimg2total!!!!!!!!!!! It sets negative values to zero NB!!!!!!!!!
While (ii lt n_elements(mass)) DO BEGIN
   If (mass(ii) lt 0) THEN mass(ii) = 0
   If (mass(ii) gt maxPIX) THEN mass(ii) = 0
...
;print,'min pixel value in ROI' + string(min(mass))    
mass = TOTAL(mass)
dte = ha.date_obs
;PRINT,'     Date-Obs       Cen-PA  Width Height ROI-Type '+ $
             ;   '(R1     R2    T1     T2      Pts) PixRad     Mass' 
 ;PRINT, dte,ac,wt,ht,roitype, r1, r2, t2, t1, np, pixrad, mass, $; cm_r, cm_a, $
